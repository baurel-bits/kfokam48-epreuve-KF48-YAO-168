...
> **EF4** (remplacement de lien) ne touche **qu'une colonne** : `exercice.lien`. Rien n'est créé ni supprimé, et le statut reste inchangé — la seule situation où RG11 l'autorise est `DEPOSE`, faute de relecteur éligible au dépôt (EF5). Le contrôle porte sur l'existence d'une ligne `relecture` pour cet exercice (`existsByExerciceId`) : c'est la vérité de « le lien n'est plus remplaçable », l'équivalence avec `statut = DEPOSE` n'étant qu'un reflet.
