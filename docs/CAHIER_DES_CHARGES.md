...
puis répercutés dans `api/contrat.yaml` — ou **écartés explicitement** lorsque la décision est de ne
rien y ajouter.
...
| **EF11** — reclôturer une session déjà clôturée | Le contrat ne déclare que `200` et `404` sur `POST /api/sessions/{id}/cloture` : aucun statut n'était prévu pour une seconde clôture. | **Idempotence** : la seconde clôture répond `200` avec `cloturee: true`, sans réécrire la session. Un `409` aurait été ambigu, ce code servant déjà à refuser une action **sur** une session clôturée. Précision de périmètre : RG14 gèle les **dépôts et les notes**, pas les présences — le contrat est explicite (« gèle tout dépôt et toute notation ») et l'issue de référence ne mentionne que ces deux points. |
| **Sessions déjà ouvertes** — aucune relecture possible | Le contrat n'expose **aucune** opération qui relirait une session ou les listerait (`/api/sessions` n'a que `POST`) : le code de présence n'existe que dans la réponse de création, et un rechargement de page le perdait alors qu'il reste valable 15 min (RG1). | **Aucune opération ajoutée** : les opérations additionnelles du contrat sont réservées « à un EF du cahier des charges ou à un prérequis explicite du sujet », et devaient être déclarées avant le premier commit de code. Le code est donc **mémorisé par le navigateur** du formateur (stockage local) et réaffiché au rechargement. Conséquence assumée : il ne revoit que les sessions qu'il a ouvertes depuis ce navigateur. La validité d'un code reste décidée par le serveur (`410 CODE_EXPIRE`) ; l'écran n'en recalcule rien (F3). |
| **EF4** — « tant qu'aucune relecture n'a été **rendue** » | L'ébauche d'issue ouvrait le remplacement de lien pendant tout `EN_ATTENTE` et ne le fermait qu'à la note rendue. Or **RG11 (Q13)**, que l'issue cite elle-même en référence, parle d'« aucune relecture **commencée** », et le contrat déclare sur cette opération `409 RELECTURE_COMMENCEE` — jamais `RELECTURE_RENDUE`. | **RG11 et le contrat font foi** : une relecture est « commencée » dès son assignation au dépôt (EF5), donc **le remplacement est refusé dès qu'un relecteur a été tiré**. Conséquence assumée : l'EF4 n'est atteignable que pour un exercice resté `DEPOSE` (déposé sans autre étudiant présent). Le modèle ne connaît que `EN_ATTENTE` et `RENDUE` : il n'existe aucun état « relecture entamée mais non rendue » qui aurait donné un sens plus large à « commencée ». |
| **EF7** — corriger une relecture jamais rendue | Le contrat déclare `400`, `403`, `404` et `409 SESSION_CLOTUREE` sur `PUT /api/relectures/{id}/correction`, mais **aucun statut** pour une relecture encore `EN_ATTENTE`. | **`409 RELECTURE_NON_RENDUE`**, symétrique du `409 RELECTURE_DEJA_RENDUE` de l'EF6 : il n'y a rien à corriger, c'est l'EF6 (`POST /api/relectures/{id}`) qui s'applique — et laisser passer produirait une écriture impossible, `correction_relecture.ancienne_note` étant `NOT NULL`. Côté interface, la correction n'est proposée que pour la relecture **qui vient d'être rendue** : `GET /api/relecteurs/{etudiantId}/relectures-en-attente` ne renvoie, par définition contractuelle, que les relectures non rendues. |
| **EF10** — étudiant inconnu : `404` ici, `400` sur l'opération imposée | Le contrat déclare `ETUDIANT_INCONNU` en **`404`** sur `POST /api/presences/manuelles`, alors que la même violation est un **`400`** sur l'opération imposée `POST /api/presences`. | Chaque opération suit **sa propre déclaration** (B2) : le `404` sur la première, le `400` sur la seconde. Aucun code d'erreur ajouté, aucune règle métier dupliquée — c'est le même contrôle, exposé par deux guichets aux statuts différents. À noter aussi : le `403` qu'envisageait l'ébauche d'issue est **abandonné** (aucune authentification, Q1), et la `source` n'est **jamais** reçue du client : elle est décidée par l'opération appelée (RG12). |
| **EF9** — portée et autorisation du tableau de bord | L'ébauche d'issue décrivait un tableau **par session** (`GET /api/tableau?sessionId=X`) et un `403 ACCES_REFUSE` empêchant un formateur de consulter la session d'un autre. | L'opération **imposée** prend **`promotionId`** et renvoie **une ligne par étudiant** : elle est reproduite telle quelle (B2), le `?sessionId=` d'ébauche est abandonné. Le `403` l'est aussi : sans authentification (Q1) il n'existe aucune notion de « sa » promotion, et le contrat ne déclare aucun `403` sur cette opération. Enfin, `relecturesEnAttente` est défini **côté auteur** (`auteurId` + `EN_ATTENTE`) : c'est la lecture de RG9, cohérente avec la « moyenne des notes reçues » de la même ligne. |
